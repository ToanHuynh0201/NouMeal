...
const reactionIcons: Record<ReactionType, { icon: any; label: string; colorScheme: string }> = {
...
export const PostCard = ({ post, onReactionUpdate }: PostCardProps) => {
  ...
  const handleReaction = async (reactionType: ReactionType) => {
    ...
    setIsLoading(true);
    try {
      const updatedPost = await communityService.toggleReaction(post.id, reactionType);
      if (updatedPost) {
  ...
  const totalReactions = currentPost.reactions.reduce((sum, r) => sum + r.count, 0);
  ...
  return (
    ...
    >
      ...
      {currentPost.images.length > 0 && (
        <Grid
          templateColumns={currentPost.images.length === 1 ? '1fr' : 'repeat(2, 1fr)'}
          gap={1}
        >
              ...
              <Box>
                ...
                <List spacing={1}>
                  {currentPost.ingredients.map((ingredient, index) => (
                    <ListItem key={index} fontSize="sm" color={mutedTextColor}>
                      • {ingredient}
                    </ListItem>
                  ))}
                </List>
      ...
      <Box px={4} py={2} borderTop="1px" borderColor={borderColor}>
        <Text fontSize="sm" color={mutedTextColor}>
          {totalReactions > 0 ? `${totalReactions} lượt thả cảm xúc` : 'Chưa có cảm xúc nào'}
        </Text>
      </Box>
      ...
      <Box px={4} py={3} borderTop="1px" borderColor={borderColor}>
        <HStack spacing={2} justify="space-around">
          {(Object.keys(reactionIcons) as ReactionType[]).map((reactionType) => {
            const reaction = currentPost.reactions.find((r) => r.type === reactionType);
