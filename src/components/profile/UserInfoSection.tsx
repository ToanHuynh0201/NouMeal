...
const UserInfoSection = () => {
	...
	const bmi =
		user.height && user.weight ? calculateBMI(user.weight, user.height) : 0;
	const bmr =
	...
	const togglePreference = (preference: string) => {
		if (editedData.preferences.includes(preference)) {
			setEditedData({
				...editedData,
				preferences: editedData.preferences.filter(
					(p) => p !== preference,
				),
			});
		} else {
							...
							{isEditing ? (
								<VStack
									...
									<Text
										fontSize="sm"
										color="gray.600">
										Click on your typical daily activity
										level:
									</Text>
									<VStack
										spacing={3}
						...
						<Box>
							...
							{isEditing ? (
								<VStack
									...
									<Wrap spacing={3}>
										{DIETARY_PREFERENCES.map((pref) => {
											const isSelected =
												editedData.preferences.includes(
													pref.value,
												);
											return (
								...
								<Wrap spacing={3}>
									...
									displayData.preferences.length > 0 ? (
										displayData.preferences.map(
											(prefValue: string) => {
												const pref =
							...
							{isEditing ? (
								<VStack
									...
									<Text
										fontSize="sm"
										color="gray.600">
										Click to select foods you are allergic
										to:
									</Text>
									<Wrap spacing={3}>
										{ALLERGIES.map((allergen) => {
											const isSelected =
												editedData.allergies.includes(
													allergen.value,
												);
											return (
								...
								<Wrap spacing={3}>
									...
									displayData.allergies.length > 0 ? (
										displayData.allergies.map(
											(allergenValue: string) => {
												const allergen = ALLERGIES.find(
													(a) =>
														a.value ===
														allergenValue,
												);
